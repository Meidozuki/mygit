...
class GitObjectsManager {
 ...
 public:
    ...
    std::optional<std::string> getFilePath(HashArg hash) const {
        if (find(hash)) {
            return std::string(".mgit/objects/") + hash;
        }
        else {
            return {};
        }
    ...
    std::string getFilePath(const IteratorType &iterator) const {
        if (iterator != set_.end()) {
            return std::string(".mgit/objects/") + *iterator;
        }
        else {
            return {};
        }
